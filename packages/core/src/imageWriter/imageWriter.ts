...
import { StreamdeckImageHeaderGenerator, StreamdeckImageWriter } from './types'
...
export class StreamdeckOriginalImageWriter implements StreamdeckImageWriter {
	...
	public generateFillImageWrites(keyIndex: number, byteBuffer: Buffer): Buffer[] {
		...
		this.headerGenerator.writeFillImageCommandHeader(packet1, keyIndex, 0x01, false, packet1Bytes)
		byteBuffer.copy(packet1, PACKET_HEADER_LENGTH, 0, packet1Bytes)

		const packet2 = Buffer.alloc(MAX_PACKET_SIZE)
		this.headerGenerator.writeFillImageCommandHeader(packet2, keyIndex, 0x02, true, packet1Bytes)
...
export class StreamdeckDefaultImageWriter implements StreamdeckImageWriter {
	private readonly headerGenerator: StreamdeckImageHeaderGenerator

	constructor(headerGenerator: StreamdeckImageHeaderGenerator) {
