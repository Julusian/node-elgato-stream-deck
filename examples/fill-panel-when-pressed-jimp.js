...
;(async () => {
	...
	const bmpImgField = await Jimp.read(path.resolve(__dirname, 'fixtures/sunny_field.png'))
		.then(img => {
			return img
				.resize(streamDeck.ICON_SIZE * streamDeck.KEY_COLUMNS, streamDeck.ICON_SIZE * streamDeck.KEY_ROWS)
		})
	const bmpImgMosaic = await Jimp.read(path.resolve(__dirname, '../src/__tests__/fixtures/mosaic.png'))
		.then(img => {
			return img
				.resize(streamDeck.ICON_SIZE * streamDeck.KEY_COLUMNS, streamDeck.ICON_SIZE * streamDeck.KEY_ROWS)
		})
